...
void ECUClass::_readFromECU() {
  ...
  switch (_readState) {
    case READ_IDLE: {
      // Find next parameter to read
      for (int i = 0; i < _numParams; i++) {
        int idx = (_currentParamIndex + i) % _numParams;
        if (!ecuParams[idx].enabled) continue;
        
        if (now - _lastPollTime[idx] >= ecuParams[idx].pollRate) {
    ...
    case READ_WAITING_RESPONSE: {
      if (now - _readStartTime >= SIM_ECU_RESPONSE_MS) {
        ...
        // Get simulated value
        int baseVal = simData[_simFrameIndex][_currentParamIndex];
        int noise = random(-2, 3);
        int result = baseVal + noise;
        result = (result < 0) ? 0 : (result > 255) ? 255 : result;
        
        DataServer.sendData(ecuParams[_currentParamIndex].name, result, (float)result, "raw");
        _lastPollTime[_currentParamIndex] = now;
        
        _currentParamIndex = (_currentParamIndex + 1) % _numParams;
        _readState = READ_IDLE;
      }
      break;
    }
...
int ECUClass::_ecuRead(unsigned int addr) {
  ...
  for (int i = 0; i < _numParams; i++) {
    if (ecuParams[i].addr == addr) {
      int baseVal = simData[_simFrameIndex][i];
      int noise = random(-2, 3);
      int result = baseVal + noise;

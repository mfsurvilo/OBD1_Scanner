...
bool ScalingConfigClass::setScaling(const char* paramName, float multiplier, float offset) {
    int idx = _findIndex(paramName);
    if (idx < 0) {
    ...
    Log.infof("ScalingConfig: Set %s mult=%.4f off=%.2f", paramName, multiplier, offset);
    return true;
}

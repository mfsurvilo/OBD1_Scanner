...
String Logger::getFlashLog() {
  ...
  if (_logMutex) xSemaphoreTake(_logMutex, portMAX_DELAY);
  File f = LittleFS.open(FLASH_LOG_FILE, "r");
  String result = f ? f.readString() : String("No log file");
  if (f) f.close();
  if (_logMutex) xSemaphoreGive(_logMutex);
...
void Logger::clearRAM() {
  ...
  _ramWrapped = false;
  if (_logMutex) xSemaphoreGive(_logMutex);
  info("RAM log cleared");  // logged outside the lock (non-recursive mutex)
}
...
void Logger::clearFlash() {
  ...
  if (_logMutex) xSemaphoreGive(_logMutex);
  if (cleared) info("Flash log cleared");  // logged outside the lock
}

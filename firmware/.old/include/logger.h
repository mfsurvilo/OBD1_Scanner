...
private:
  ...
  // Guards the shared buffers/flash file: the ECU runs on its own core and
  // logs concurrently with the main loop. Non-recursive, so the internal
  // info() calls in clearRAM/clearFlash are made AFTER releasing it.
